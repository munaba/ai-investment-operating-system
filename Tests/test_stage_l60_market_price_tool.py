"""
Phase 5 Sprint 60 proof suite -- ``MarketPriceTool``, the project's
first real trading Tool.

Scope: dedicated regression suite for
``Orchestration.market_price_tool.MarketPriceTool`` only. This sprint
is interface-level only -- no real market-data operation exists to
test, and this suite spends most of its effort proving the *absence*
of any such operation (network/API/Service/Repository/Provider/
Database access, and any runtime/workflow/executor/registry/skill
knowledge) just as much as it proves the presence of the three
required ``BaseTool`` members.

Invariant coverage:
    M1  -- MarketPriceTool is a subclass of BaseTool.
    M2  -- MarketPriceTool satisfies BaseTool's abstract contract --
           it constructs with zero arguments and does not raise
           TypeError.
    M3  -- MarketPriceTool().name == "market_price".
    M4  -- MarketPriceTool().description ==
           "Retrieve market price information." (exact match).
    M5  -- MarketPriceTool().execute(context) raises
           NotImplementedError for any context value (None, a string,
           a dict, an arbitrary object).
    M6  -- MarketPriceTool defines no __init__ of its own -- it
    ...
    M8  -- none of the forbidden domain methods (fetch, download,
           request, query, history, latest, price, ohlcv, candles,
           ticker, quote, stream, subscribe, connect, disconnect,
           refresh, update, reset, validate, build, prepare) exist on
           the class or an instance.
    M9  -- Orchestration/market_price_tool.py's source contains no
           reference to any forbidden import (requests, httpx,
           aiohttp, urllib, pandas, numpy, yfinance, ccxt, polygon,
           alpaca, finnhub, binance, sqlite3, json, pathlib, os, time,
           datetime, Repository, Services, Providers, Database,
           Agents, Planner, Executor, Workflow, Runtime, EventBus,
           Memory, LearningLoop, Reflection, CompositionRoot,
           ToolRegistry, ToolResolver, ToolManager, Skill, Capability)
    ...
    M12 -- MarketPriceTool carries no runtime/workflow/executor/
    ...
    M13 -- isinstance/issubclass relationships hold as expected:
           ...
           between MarketPriceTool and BaseTool is not symmetric.
    M14 -- MarketPriceTool defines no public attribute/method beyond
           the three BaseTool-required members (name, description,
           execute).
    M15 -- no class variables of any kind (MarketPriceTool's own
    ...
    M16 -- multiple instances independent: two MarketPriceTool
           instances are distinct objects that never share mutable
           state (there is none to share).
    M17 -- side-effect free: constructing MarketPriceTool and reading
           name/description repeatedly is idempotent and produces no
           observable side effects.
    M18 -- AST import verification: Orchestration/market_price_tool.py
...
def scenario_inherits_base_tool() -> None:
    check(
        issubclass(MarketPriceTool, BaseTool),
        "M1: MarketPriceTool subclasses BaseTool",
    )
...
def scenario_abstract_contract_satisfied() -> None:
    ...
    check(
        constructed,
        "M2: MarketPriceTool() constructs without TypeError -- the "
...
# ---------------------------------------------------------------------------
# M3 -- name == "market_price"
# ---------------------------------------------------------------------------
def scenario_name_is_market_price() -> None:
    tool = MarketPriceTool()
    check(
        tool.name == "market_price",
        f"M3: MarketPriceTool().name == 'market_price'; got {tool.name!r}",
    )
...
def scenario_description_exact_match() -> None:
    ...
    check(
        ...
        f"M4: MarketPriceTool().description == 'Retrieve market price "
        f"information.'; got {tool.description!r}",
    )
...
def scenario_no_init_of_its_own() -> None:
    check(
        "__init__" not in MarketPriceTool.__dict__,
        "M6: MarketPriceTool does not define its own __init__",
    )
...
def scenario_no_instance_state() -> None:
    ...
    check(
        ...
        "M7: a constructed MarketPriceTool instance carries no "
...
def scenario_forbidden_methods_absent() -> None:
    ...
    for method in forbidden_methods:
        check(
            not hasattr(MarketPriceTool, method) and not hasattr(tool, method),
...
def scenario_no_forbidden_imports() -> None:
    import Orchestration.market_price_tool as mpt_module

    source = Path(mpt_module.__file__).read_text(encoding="utf-8")
    ...
    for forbidden in forbidden_import_modules:
        check(
            ...
            f"M9: Orchestration/market_price_tool.py contains no "
    ...
    for symbol in forbidden_symbols:
        ...
        check(
            not hasattr(mpt_module, symbol),
            f"M10: Orchestration.market_price_tool's module namespace "
...
def scenario_no_filesystem_or_network_side_effects() -> None:
    ...
    def _blocked_socket(*args, **kwargs):  # noqa: ANN001, ANN002, ANN003
        ...
        socket_called = True
        raise AssertionError("MarketPriceTool attempted to open a network socket")
    ...
    check(
        before == after,
        "M11: constructing MarketPriceTool and calling its (failing) "
...
def scenario_no_runtime_workflow_executor_registry_knowledge() -> None:
    ...
    for attr in (
        ...
        check(
            not hasattr(MarketPriceTool, attr) and not hasattr(tool, attr),
            f"M12: neither MarketPriceTool nor an instance carries a "
            f"'{attr}' attribute",
        )
...
def scenario_isinstance_issubclass_relationships() -> None:
    ...
    check(
        isinstance(tool, BaseTool),
        "M13: isinstance(MarketPriceTool(), BaseTool) is True",
    )
    check(
        issubclass(MarketPriceTool, BaseTool),
        "M13: issubclass(MarketPriceTool, BaseTool) is True",
    )
    check(
        not issubclass(BaseTool, MarketPriceTool),
        "M13: BaseTool is not a subclass of MarketPriceTool -- the "
...
def scenario_no_extra_public_surface() -> None:
    ...
    public_class_members = {
        member for member in vars(MarketPriceTool) if not member.startswith("_")
    }
    check(
        public_class_members == allowed,
        f"M14: MarketPriceTool's own public class namespace is exactly "
...
def scenario_no_class_variables() -> None:
    ...
    for key, value in vars(MarketPriceTool).items():
        if key.startswith("_"):
            # Skip dunder attributes and ABCMeta-injected bookkeeping
            # (e.g. '_abc_impl') -- these are machinery from `abc.ABC`
            # itself, not a class variable MarketPriceTool defines.
            continue
        check(
            key in allowed_names,
            f"M15: MarketPriceTool's class namespace has no extra "
            f"class variable beyond the three required members; found {key!r}",
        )
        check(
            isinstance(value, (property, type(MarketPriceTool.execute))) or callable(value),
...
def scenario_multiple_instances_independent() -> None:
    a = MarketPriceTool()
    b = MarketPriceTool()
    check(a is not b, "M16: two MarketPriceTool instances are distinct objects")
    check(
        a.name == b.name and a.description == b.description,
...
def scenario_ast_import_verification() -> None:
    source_path = ROOT / "Orchestration" / "market_price_tool.py"
    ...
    check(
        not unexpected,
        f"M18: Orchestration.market_price_tool imports only allowed "
        f"modules; unexpected={unexpected!r}",
    )
    check(
        "Orchestration.base_tool" in imported_modules,
        "M18: Orchestration.market_price_tool imports Orchestration.base_tool",
    )
...
def main() -> int:
    ...
    print("\n" + "=" * 60)
    print(f"PHASE 5 SPRINT 60 MARKET PRICE TOOL RESULTS: {_PASS} PASS / {_FAIL} FAIL (total {_PASS + _FAIL})")
    print("=" * 60)
    if _FAILURES:
        print("Failed checks:")
        for f in _FAILURES:
