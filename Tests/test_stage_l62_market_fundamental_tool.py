"""
Phase 5 Sprint 62 proof suite -- ``MarketFundamentalTool``, the project's
third concrete Tool.

Scope: dedicated regression suite for
``Orchestration.market_fundamental_tool.MarketFundamentalTool`` only.
This sprint is interface-level only -- no real financial-statement or
fundamentals-retrieval operation exists to test, and this suite spends
most of its effort proving the *absence* of any such operation
(network/API/SEC-EDGAR/Service/Repository/Provider/Database access,
and any runtime/workflow/executor/registry knowledge) just as much as
it proves the presence of the three required ``BaseTool`` members.

Invariant coverage:
    M1  -- MarketFundamentalTool is a subclass of BaseTool.
    M2  -- MarketFundamentalTool satisfies BaseTool's abstract contract --
           it constructs with zero arguments and does not raise
           TypeError.
    M3  -- MarketFundamentalTool().name == "market_fundamental".
    M4  -- MarketFundamentalTool().description ==
           "Retrieve market fundamental information." (exact match).
    M5  -- MarketFundamentalTool().execute(context) raises
           NotImplementedError for any context value (None, a string,
           a dict, an arbitrary object).
    M6  -- MarketFundamentalTool defines no __init__ of its own -- it
    ...
    M8  -- none of the forbidden domain methods (financials,
           balance_sheet, income_statement, cash_flow, ratios,
           valuation, earnings, eps, revenue, assets, liabilities,
           equity, sec, edgar, download, fetch, query, request,
           refresh) exist on the class or an instance.
    M9  -- Orchestration/market_fundamental_tool.py's source contains
           no reference to any forbidden import (requests, httpx,
           aiohttp, pandas, numpy, yfinance, sec_edgar, sec_api,
           polygon, alphavantage, financialmodelingprep, sqlite3,
           Repository, Services, Providers, Database, Planner,
           Executor, Workflow, Runtime, EventBus, Memory,
           LearningLoop, Reflection, CompositionRoot, ToolRegistry,
           ToolResolver, ToolManager) -- verified both by substring
           scan and AST-level import inspection.
    ...
    M12 -- MarketFundamentalTool carries no runtime/workflow/executor/
    ...
    M13 -- isinstance/issubclass relationships hold as expected:
           ...
           between MarketFundamentalTool and BaseTool is not symmetric.
    M14 -- MarketFundamentalTool defines no public attribute/method beyond
           the three BaseTool-required members (name, description,
           execute).
    M15 -- no class variables of any kind (MarketFundamentalTool's own
    ...
    M16 -- multiple instances independent: two MarketFundamentalTool
           instances are distinct objects that never share mutable
           state (there is none to share).
    M17 -- side-effect free: constructing MarketFundamentalTool and reading
           name/description repeatedly is idempotent and produces no
           observable side effects.
    M18 -- AST import verification: Orchestration/market_fundamental_tool.py
...
def scenario_inherits_base_tool() -> None:
    check(
        issubclass(MarketFundamentalTool, BaseTool),
        "M1: MarketFundamentalTool subclasses BaseTool",
    )
...
def scenario_abstract_contract_satisfied() -> None:
    ...
    check(
        constructed,
        "M2: MarketFundamentalTool() constructs without TypeError -- the "
...
# ---------------------------------------------------------------------------
# M3 -- name == "market_fundamental"
# ---------------------------------------------------------------------------
def scenario_name_is_market_fundamental() -> None:
    tool = MarketFundamentalTool()
    check(
        tool.name == "market_fundamental",
        f"M3: MarketFundamentalTool().name == 'market_fundamental'; got {tool.name!r}",
    )
...
def scenario_description_exact_match() -> None:
    ...
    check(
        ...
        f"M4: MarketFundamentalTool().description == 'Retrieve market "
        f"fundamental information.'; got {tool.description!r}",
    )
...
def scenario_no_init_of_its_own() -> None:
    check(
        "__init__" not in MarketFundamentalTool.__dict__,
        "M6: MarketFundamentalTool does not define its own __init__",
    )

    a = MarketFundamentalTool()
    b = MarketFundamentalTool()
    check(
        a is not b,
...
def scenario_no_instance_state() -> None:
    ...
    check(
        ...
        "M7: a constructed MarketFundamentalTool instance carries no "
...
def scenario_forbidden_methods_absent() -> None:
    ...
    for method in forbidden_methods:
        check(
            not hasattr(MarketFundamentalTool, method) and not hasattr(tool, method),
...
def scenario_no_forbidden_imports() -> None:
    import Orchestration.market_fundamental_tool as mft_module

    source = Path(mft_module.__file__).read_text(encoding="utf-8")
    ...
    for forbidden in forbidden_import_modules:
        check(
            ...
            f"M9: Orchestration/market_fundamental_tool.py contains no "
    ...
    for symbol in forbidden_symbols:
        ...
        check(
            not hasattr(mft_module, symbol),
            f"M10: Orchestration.market_fundamental_tool's module namespace "
...
def scenario_no_filesystem_or_network_side_effects() -> None:
    ...
    def _blocked_socket(*args, **kwargs):  # noqa: ANN001, ANN002, ANN003
        ...
        socket_called = True
        raise AssertionError("MarketFundamentalTool attempted to open a network socket")
    ...
    check(
        before == after,
        "M11: constructing MarketFundamentalTool and calling its (failing) "
...
def scenario_no_runtime_workflow_executor_registry_knowledge() -> None:
    ...
    for attr in (
        ...
        check(
            not hasattr(MarketFundamentalTool, attr) and not hasattr(tool, attr),
            f"M12: neither MarketFundamentalTool nor an instance carries a "
            f"'{attr}' attribute",
        )
...
def scenario_isinstance_issubclass_relationships() -> None:
    ...
    check(
        isinstance(tool, BaseTool),
        "M13: isinstance(MarketFundamentalTool(), BaseTool) is True",
    )
    check(
        issubclass(MarketFundamentalTool, BaseTool),
        "M13: issubclass(MarketFundamentalTool, BaseTool) is True",
    )
    check(
        not issubclass(BaseTool, MarketFundamentalTool),
        "M13: BaseTool is not a subclass of MarketFundamentalTool -- the "
...
def scenario_no_extra_public_surface() -> None:
    ...
    public_class_members = {
        member for member in vars(MarketFundamentalTool) if not member.startswith("_")
    }
    check(
        public_class_members == allowed,
        f"M14: MarketFundamentalTool's own public class namespace is exactly "
...
def scenario_no_class_variables() -> None:
    ...
    for key, value in vars(MarketFundamentalTool).items():
        if key.startswith("_"):
            # Skip dunder attributes and ABCMeta-injected bookkeeping
            # (e.g. '_abc_impl') -- these are machinery from `abc.ABC`
            # itself, not a class variable MarketFundamentalTool defines.
            continue
        check(
            key in allowed_names,
            f"M15: MarketFundamentalTool's class namespace has no extra "
            f"class variable beyond the three required members; found {key!r}",
        )
        check(
            isinstance(value, (property, type(MarketFundamentalTool.execute))) or callable(value),
...
def scenario_multiple_instances_independent() -> None:
    a = MarketFundamentalTool()
    b = MarketFundamentalTool()
    check(a is not b, "M16: two MarketFundamentalTool instances are distinct objects")
    check(
        a.name == b.name and a.description == b.description,
...
def scenario_ast_import_verification() -> None:
    source_path = ROOT / "Orchestration" / "market_fundamental_tool.py"
    ...
    check(
        not unexpected,
        f"M18: Orchestration.market_fundamental_tool imports only allowed "
        f"modules; unexpected={unexpected!r}",
    )
    check(
        "Orchestration.base_tool" in imported_modules,
        "M18: Orchestration.market_fundamental_tool imports Orchestration.base_tool",
    )
...
def main() -> int:
    scenarios = [
        ...
        scenario_name_is_market_fundamental,
    ...
    print("\n" + "=" * 60)
    print(f"PHASE 5 SPRINT 62 MARKET FUNDAMENTAL TOOL RESULTS: {_PASS} PASS / {_FAIL} FAIL (total {_PASS + _FAIL})")
    print("=" * 60)
    if _FAILURES:
        print("Failed checks:")
        for f in _FAILURES:
