"""Phase 8 Sprint 90 -- ``MarketNewsTool.execute()`` ``ToolResult``
contract.
...
mirroring ``Tests/test_stage_l89_market_price_tool_tool_result.py``.
"""
...
def scenario_return_annotation() -> None:
    ...
    check(
        not getattr(MarketNewsTool.execute, "__isabstractmethod__", False),
        "T1: execute() remains a concrete (non-abstract) method on MarketNewsTool",
    )
...
def scenario_execute_still_raises() -> None:
    ...
    raised3 = False
    try:
        MarketNewsTool.execute(tool, object())
    except NotImplementedError:
        raised3 = True
...
def scenario_name_and_description_unchanged() -> None:
    tool = MarketNewsTool()
    check(tool.name == "market_news", "T3: name is still the literal string 'market_news'")
    check(
        tool.description == "Retrieve market news.",
        "T3: description is still the literal string 'Retrieve market news.'",
    )
    check(isinstance(type(MarketNewsTool.name), type(property)), "T3: 'name' is still a property")
    check(isinstance(type(MarketNewsTool.description), type(property)), "T3: 'description' is still a property")
    check(
        not getattr(MarketNewsTool.name.fget, "__isabstractmethod__", False),
        "T3: 'name' is concrete (not abstract) on MarketNewsTool",
    )
    check(
        not getattr(MarketNewsTool.description.fget, "__isabstractmethod__", False),
        "T3: 'description' is concrete (not abstract) on MarketNewsTool",
    )


# T4 -- no constructor, no attributes, no helpers, no class variables
def scenario_no_constructor_no_helpers() -> None:
    check("__init__" not in vars(MarketNewsTool), "T4: MarketNewsTool still defines no __init__ of its own")

    tool = MarketNewsTool()
    check(not hasattr(tool, "__dict__") or tool.__dict__ == {}, "T4: instance carries no instance attributes")

    public_members = {
        n for n in dir(MarketNewsTool)
        if not n.startswith("_")
        and (callable(vars(MarketNewsTool).get(n) or getattr(MarketNewsTool, n, None))
             or isinstance(vars(MarketNewsTool).get(n), property))
    }
    check(
    ...
    forbidden_methods = (
        "fetch", "download", "query", "search", "rss", "scrape",
        "crawl", "headline", "articles", "news", "provider", "request",
        "refresh", "stream", "connect", "disconnect", "parse",
        "summarize", "classify", "cache", "retry",
    )
    check(
        not any(hasattr(MarketNewsTool, m) for m in forbidden_methods),
        f"T4: MarketNewsTool has none of the forbidden methods {forbidden_methods!r}",
    )

    check(
        not any(
            k for k in vars(MarketNewsTool)
            if not k.startswith("__") and k not in ("name", "description", "execute", "_abc_impl")
        ),
        "T4: no additional class variable was added to MarketNewsTool",
    )


# T5 -- MarketNewsTool still subclasses BaseTool, instantiable
def scenario_subclass_and_instantiation() -> None:
    check(issubclass(MarketNewsTool, BaseTool), "T5: MarketNewsTool still subclasses BaseTool")
    try:
        MarketNewsTool()
        ok = True
    except Exception:  # noqa: BLE001
        ok = False
    check(ok, "T5: MarketNewsTool is still freely instantiable (all abstract members satisfied)")

    tool_a, tool_b = MarketNewsTool(), MarketNewsTool()
    check(tool_a is not tool_b, "T5: two MarketNewsTool() instances are distinct objects")
    check(tool_a.name == tool_b.name == "market_news", "T5: both instances report the same stable name")


# T6 -- AST verification: exactly the expected import set, source-level checks
def scenario_ast_verification() -> None:
    import Orchestration.market_news_tool as mod
    ...
    scraping_tokens = ("requests", "feedparser", "newspaper", "BeautifulSoup", "bs4")
    architecture_tokens = (
        "Provider", "Repository", "Service", "Runtime", "Workflow",
        "Executor", "Planner", "Registry", "Resolver", "ToolManager",
    )
    check(
        not any(any(tok in t for t in imported_tokens) for tok in scraping_tokens),
        f"T6: no import statement references any scraping token {scraping_tokens!r}",
    )
    check(
    ...
    operational_tokens = ("isinstance(", "logging", "logger", "retry", ".scrape(")
    check(
...
def scenario_namespace_verification() -> None:
    ...
    check(hasattr(mod, "MarketNewsTool"), "T7: module namespace exposes MarketNewsTool")
...
def main() -> int:
    ...
    print("\n" + "=" * 60)
    print(f"PHASE 8 SPRINT 90 MARKET NEWS TOOL TOOL RESULT RESULTS: {_PASS} PASS / {_FAIL} FAIL (total {_PASS + _FAIL})")
    print("=" * 60)
    if _FAILURES:
        print("Failed checks:")
        for f in _FAILURES:
