...
test.describe('EquityChart screenshots', () => {
  ...
  test('zero baseline dashed line visible in DOM', async ({ page }) => {
    await gotoEquity(page);
    // Bklit Grid renders highlight row as <line> with stroke rgba(212,255,63,.22) dash 3 4
    ...
    const count = await page.evaluate(() => {
      ...
      return lines.filter(l => {
        ...
        return s.includes('212,255,63') && d.includes('3');
      }).length;
    });
    expect(count, 'zero baseline dashed line (lime 3 4) should exist').toBeGreaterThanOrEqual(1);
    // also verify a11y
